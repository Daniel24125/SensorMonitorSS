...
const DashboardCard = ({title, children, subtitle, secondaryAction, setSelected, color, active}: DashboardCardPropsType) => {
    const parsedTitle = typeof title === "string" && title.length > 36 ?  `${title.substring(0, 36)}...` : title
    return (<div style={{
        border: active ? `solid 2px ${color}`: "none"
    }} onClick={()=>setSelected ? setSelected(): false} className='bg-card w-full h-20 p-3 rounded-xl flex mb-5 hover:bg-card-hover cursor-pointer'>
